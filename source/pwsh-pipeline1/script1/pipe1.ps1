...
function Read-Source
{
    ...
    process
    {
        ...
        if($count)
        {
            cat $file -First $count | ?{![string]::IsNullOrWhiteSpace($_)}
        }
        else
        {
...
#Read-Source -file C:\temp\source1.txt -count 1 | Read-Pipe -Tag 'A' | Read-Pipe -Tag 'B'
#Read-Source -file C:\temp\source1.txt -count 1 | Read-Pipe -Tag 'A' | Read-Pipe -Tag 'B' | Read-Pipe -Tag 'C'
Read-Source -file C:\temp\source1.txt -count 1 | Read-Pipe -Tag 'A' -T | Read-Pipe -Tag 'B' -T | Read-Pipe -Tag 'C' -T
